public class Solution {
    public bool IsIsomorphic(string s, string t) {
        IDictionary<int, int> hm1 = new Dictionary<int, int>();
        IDictionary<int, int> hm2 = new Dictionary<int, int>();
        for (int x = 0; x < s.Length; x++) {
            if (((hm1.ContainsKey(t[x])) && (s[x] != hm1[t[x]])) || ((hm2.ContainsKey(s[x])) && (t[x] != hm2[s[x]]))) {
                return false;
            } else {
                hm2[s[x]] = t[x];
                hm1[t[x]] = s[x];
